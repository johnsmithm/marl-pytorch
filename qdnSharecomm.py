...
class DQN(nn.Module):
    ...
    def __init__(self, h, w):
        ...
        self.comm = nn.Linear(4, pars['en']) # 448 or 512
        self.commo = nn.Linear(pars['en'], 4) # 448 or 512
    ...
    def forward(self, x, agent_index, comm):
        ...
        x = F.relu(self.head(x.view(x.size(0), -1))) + F.relu(self.comm(comm.float()))
        return self.head1(x), self.commo(x)
...
def get_screen(ag=None):
    # Returned screen requested by gym is 400x600x3, but is sometimes larger
    # such as 800x1200x3. Transpose it into torch order (CHW).
    screen = env.train_render(ag).transpose((2, 0, 1))#env.render(mode='rgb_array').transpose((2, 0, 1))
...
def optimize_model(memory, policy_net, target_net, optimizer):
    ...
    comm = policy_net(state1_batch, 1, mes)[idC] if np.random.rand()<prob else mes
    if pars['comm'] =='2':
        comm = comm.detach()
    ...
    #todo: use the comm from other agent here too
    next_state_values[non_final_mask] = target_net(non_final_next_states, 1, mes)[0].max(1)[0].detach()
...
def main(pars, callback=None):
    ...
    for i_episode in range(num_episodes):
        ...
        state1 = get_screen(0)# current_screen - last_screen
        state2 = get_screen(1)# current_screen - last_screen
        rt = 0; ac=[]
        ...
        for t in range(pars['epsteps']):# count():
            ...
            with torch.no_grad():
                ...
                comm1 = policy_net(state1, 0, mes)[idC].detach() if np.random.rand()<prob else torch.tensor([[0,0.,0,0]], device=device)
                comm2 = policy_net(state2, 0, mes)[idC].detach() if np.random.rand()<prob else torch.tensor([[0,0.,0,0]], device=device)
                action1 = select_action(state1, 0, policy_net, comm2)
                #_, reward, done, _ = env.step(action.item())
                action2 = select_action(state2, 1, policy_net, comm1)
            ...
            next_state1 = get_screen(0)#
            next_state2 = get_screen(1)#
            buf1.append([mark(state1), action1, next_state1, reward1, state2])
            buf1.append([mark(state2), action2, next_state2, reward2, state1])
        ...
        if i_episode%pars['show']==0:
            ...
            for t in range(100):#pars['epsteps']):# count():
                ...
                with torch.no_grad():
                    ...
                    comm1 = policy_net(state1, 0, mes)[idC].detach() if np.random.rand()<prob else torch.tensor([[0,0.,0,0]], device=device)
                    comm2 = policy_net(state2, 0, mes)[idC].detach() if np.random.rand()<prob else torch.tensor([[0,0.,0,0]], device=device)
                    action1 = takeAc(mark(state1), 0, policy_net, comm2)# policy_net(mark(state), torch.tensor([0], device=device)).max(1)[1].view(1, 1)
                    action2 = takeAc(state2, 1, policy_net, comm1)#policy_net(state, torch.tensor([1], device=device)).max(1)[1].view(1, 1)
                #_, reward, done, _ = env.step(action.item())
                #action2 = np.random.randint(8)
                reward1, reward2 = env.move(action1.item(), action2.item())
                ep.append([env.render_env(), [action1.item(), action2.item()], [reward1, reward2],[comm1.cpu().data.numpy()[0].tolist(), comm1.cpu().data.numpy()[0].tolist()],[comm1.max(1)[1].item(), comm1.max(1)[1].item()]])
