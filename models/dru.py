...
class DRU:
	def __init__(self, sigma, comm_narrow=True, hard=False, device=None):
		self.sigma = sigma
		self.comm_narrow = comm_narrow
		self.hard = hard; self.device = device
	...
	def discretize(self, m):
		if self.hard:
			if self.comm_narrow:
				...
				m_ = torch.zeros_like(m, device=self.device)
				if m.dim() == 1:      
					_, idx = m.max(0)
					m_[idx] = 1.
				elif m.dim() == 2:      
	...
	def forward(self, m, train_mode):
		if train_mode:
			return self.regularize(m.float())
		else:
			return self.discretize(m.float())