    ...
    def sample(self, batch_size):
        ...
        for start, end in zip(begin, finish):
            ...
            while(len(final)<self.seq_length):
                final = [(np.zeros_like(self.memory[0][0]), 0, 0, np.zeros_like(self.memory[0][3]),self.memory[0][4])] + final
...
def save_episode_and_reward_to_csv(file, writer, e, r, ep, name, pars):
